...
5\. Enable them back in Discord, by going back to "plugins" or "themes" in settings, and enable them as is shown below:
...
5\. (Optional) Go to your BetterDiscord folder and remove everything from it (which can be accessed from `~/Library/Application Support/betterdiscord`). You can also delete the installer if you do not wish to use BD anymore.

</Details>

<Details summary={'"Where are my BetterDiscord folders located?"'}>
Press <kbd>⌘Command</kbd> + <kbd>Shift</kbd> + <kbd>G</kbd> in the finder to open the Go to Folder window, then paste in the path that you want to open the folder that you wish to open.
...
Open discord from application not desktop or anywhere else

If it says it's damaged, reinstall discord and open it first before installing bd again.
